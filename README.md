...
Four liveries (Delta, United, American, Singapore Airlines) via the LIGHTS × GAIN
...
Which palette colour can do a given job differs per livery — United's chrome
(HUD corners, tick rules) is remapped to its sky blue rather than its default
highlight colour, for instance. `hubrun.css` consumes `--c-highlight` and
`--c-chrome` throughout and therefore needs no per-livery overrides and no
`!important` of its own; any such remapping is the theme's job, not this
repo's.
...
name is historical. Get that wrong and a livery whose highlight has been
remapped away from its base accent will render with the wrong colour.
